...
//import com.daml.lf.engine.EngineTest.clara
import com.daml.lf.value.Value.{ContractId /*, ValueParty, ValueRecord*/}
...
case class CommandAndCont(command: Command, cont: ContractId => Seq[Command] = _ => Seq.empty)
...
final class CommandGenerator(
  ...
  def next(): Try[CommandAndCont] =
    (for {
      (contractGenDescription, observers) <- Try((pickContractGenDescription(), pickObservers()))
      createContractPayload <- Try(randomPayload(contractGenDescription.payloadSizeBytes))
      archive <- Try(pickArchive(contractGenDescription))
      command = createContractCommand(
        templateName = contractGenDescription.template,
        signatory = signatory,
        observers = observers,
  ...
  private def pickContractGenDescription(): SubmissionConfig.ContractDescription =
    ...
    CommandAndCont(
      ...
      cont = {
        def createCont(cid: ContractId): Seq[Command] = {
          ...
          // TODO pbatko handle archive (in presence and absence of consumingExercise)
