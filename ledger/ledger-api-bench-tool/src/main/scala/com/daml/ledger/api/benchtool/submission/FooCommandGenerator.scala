...
case class PartiesForContracts(
    observers: List[Primitive.Party],
    divulgees: List[Primitive.Party],
    partySetParties: List[Primitive.Party],
)

trait PartySelecting {

  def nextPartiesForContracts(): PartiesForContracts

  def nextExtraSubmitter(): List[Primitive.Party]

}
class FooRandomPartySelecting(
    config: FooSubmissionConfig,
    allocatedParties: AllocatedParties,
    randomnessProvider: RandomnessProvider,
) extends PartySelecting {
  ...
  private val partySetProbability =
    config.partySetO.fold(List.empty[(Primitive.Party, Double)]) { partySet =>
      allocatedParties.partySetParties.map(party => party -> partySet.visibility)
    }

  override def nextPartiesForContracts(): PartiesForContracts = {
    PartiesForContracts(
      observers = pickParties(observersProbability),
      divulgees = pickParties(divulgeesProbability),
      partySetParties = pickParties(partySetProbability),
    )
  }

  override def nextExtraSubmitter(): List[Primitive.Party] = pickParties(extraSubmittersProbability)
  ...
  override def next(): Try[Seq[Command]] =
    (for {
      ...
      command = createCommands(
        ...
        observers = partySelection.observers ++ partySelection.partySetParties,
