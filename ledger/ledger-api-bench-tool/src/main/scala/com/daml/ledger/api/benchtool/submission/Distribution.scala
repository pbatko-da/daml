...
class Distribution[T](weights: List[Int], items: IndexedSeq[T]) {
  ...
  def choose(randomDouble: Double): T = chooseInternal(randomDouble)._2

  private[submission] def chooseInternal(randomDouble: Double): (Int, T) = {
    ...
    val index = distribution.indexWhere(_ > randomDouble)
    (index, items(index))
  }
