...
// TODO pbatko: Test what error message is served when all sorts of operation are done on an non-existent user (a user that was just deleted).
final class UserManagementServiceIT extends LedgerTestSuite {
  ...
  test(
    ...
    // TODO pbatko: Hardcoded: max number of user rights: 1000
  ...
  test(
    ...
    def testListUserRights(): Future[Unit] = {
      for {
        ...
        res6 <- ledger.userManagement.listUserRights(ListUserRightsRequest("user4"))
      } yield {
