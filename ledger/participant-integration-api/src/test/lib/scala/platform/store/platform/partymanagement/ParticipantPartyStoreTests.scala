...
import com.daml.ledger.participant.state.index.v2.ParticipantPartyRecordStore.{ExplicitMergeUpdateWithDefaultValue, PartyRecordExists, PartyRecordNotFound}
import com.daml.ledger.participant.state.index.v2.{ObjectMetaUpdate, ParticipantPartyRecordStore, PartyRecordUpdate}
            ...
            update1 <- tested.updatePartyRecord(
              partyRecordUpdate = PartyRecordUpdate(
                ...
                metadataUpdate = ObjectMetaUpdate(
                  ...
                  annotationsUpdate = Some(Map("k1" -> "v1")),
      ...
      "should succeed when updating a non-existing party record for which a ledger party exists" in {
        testIt { tested =>
          ...
          for {
            ...
            _ = update1.value shouldBe createdPartyRecord("party1", resourceVersion = "1", annotations = Map("k1" -> "v1", "k2" -> "v2"))
          } yield succeed
        }
      }
      ...
      "should metadata annotations with merge and replace semantics" in {
        testIt { tested =>
          ...
          for {
            ...
            _ = create1.value shouldBe createdPartyRecord("party1", resourceVersion = "0", annotations = Map("k1" -> "v1", "k2" -> "v2"))
            ...
            _ = update1.value shouldBe createdPartyRecord("party1", resourceVersion = "1", annotations = Map("k1" -> "v1b", "k2" -> "v2", "k3" -> "v3"))
            ...
            _ = update2.value shouldBe createdPartyRecord("party1", resourceVersion = "2",annotations = Map("k1" -> "v1c", "k4" -> "v4"))
            ...
            update3 <- tested.updatePartyRecord(
              partyRecordUpdate = PartyRecordUpdate(
                ...
                metadataUpdate = ObjectMetaUpdate(
                  resourceVersionO = None,
                  annotationsUpdate = Some(Map.empty),
                  replaceAnnotations = true,
                ),
              ),
              ledgerPartyExists = _ => Future.successful(true),
            )
            _ = update3.value shouldBe createdPartyRecord("party1", resourceVersion = "3",annotations = Map.empty)

            // TODO pbatko: default merge: when non default values ?
            // TODO pbatko: default replace: when the default value ?
            // TODO pbatko: Raise error?
            // forth update: update to default value with merge semantics is invalid
            update4 <- tested.updatePartyRecord(
              partyRecordUpdate = PartyRecordUpdate(
                party = pr.party,
                metadataUpdate = ObjectMetaUpdate(
                  resourceVersionO = None,
            ...
            _ = update4.left.value shouldBe ExplicitMergeUpdateWithDefaultValue(pr.party)
