...
private[backend] trait ParticipantResourceMetadataTests
  ...
  it should "get, add and delete user's annotations" in {
    ...
    // Add key1
    executeSql(
      tested.addAnnotation(resourceAnnotationsTableName)(internalId, key = "key1", value = "value1", updatedAt = 123)
    )
    executeSql(tested.getAnnotations(resourceAnnotationsTableName)(internalId)) shouldBe Map("key1" -> "value1")
    // Add key2
    executeSql(
      tested.addAnnotation(resourceAnnotationsTableName)(internalId, key = "key2", value = "value2", updatedAt = 123)
    )
    ...
    assertThrows[SQLException](
      executeSql(
        tested
          .addAnnotation(resourceAnnotationsTableName)(internalId, key = "key2", value = "value2b", updatedAt = 123)
      )
    )
    // Delete
  ...
  it should "store and retrieve an empty annotation value" in {
    val resource = newResource()
    val internalId = resource.createResourceAndReturnInternalId()
    executeSql(tested.addAnnotation(resourceAnnotationsTableName)(internalId, key = "key", value = "", updatedAt = 0))
    executeSql(tested.getAnnotations(resourceAnnotationsTableName)(internalId)) shouldBe Map("key" -> "")
  }
  ...
  it should "allow to store 256kb of annotations (counted in utf-8 bytes)" in {
    ...
    (key.getBytes(StandardCharsets.UTF_8).length + value.getBytes(StandardCharsets.UTF_8).length) shouldBe 256 * 1024
    executeSql(tested.addAnnotation(resourceAnnotationsTableName)(internalId, key = key, value = value, updatedAt = 0))
  }

  it should "allow to store many annotations annotations with combined size of 256kB in utf-8 encoding" in {
    val resource = newResource()
    val internalId = resource.createResourceAndReturnInternalId()
    val longestKeyPrefix = "a" * 253
    val longestKeyName = "b" * 63
    val longestKey = s"$longestKeyPrefix/$longestKeyName"
    longestKey should have length(253 + 1 + 63)
    executeSql(tested.addAnnotation(resourceAnnotationsTableName)(internalId, key = longestKey, value = "longest key", updatedAt = 0))
  }
