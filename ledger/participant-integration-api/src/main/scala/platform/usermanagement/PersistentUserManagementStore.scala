...
import com.daml.ledger.participant.state.index.v2.UserManagementStore.{ConcurrentUserUpdateDetected, Result, TooManyUserRights, UserExists, UserInfo, UserNotFound, UsersPage}
...
import com.daml.platform.usermanagement.PersistentUserManagementStore.{ConcurrentUserUpdateDetectedRuntimeException, TooManyUserRightsRuntimeException}
    ...
    inTransaction(_.updateUser) { implicit connection =>
      for {
        _ <- withUser(id = userUpdate.id) { dbUser =>
          ...
          userUpdate.metadataUpdate.annotationsUpdate.foreach { newValue =>
            backend.deleteUserAnnotations(internalId = dbUser.internalId)(connection)
            newValue.iterator.foreach { case (key, value) =>
              backend.addUserAnnotation(
