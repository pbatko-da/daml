...
import com.daml.platform.partymanagement.PersistentParticipantPartyRecordStore.ConcurrentPartyRecordUpdateDetectedRuntimeException
import com.daml.platform.store.DbSupport
import com.daml.platform.store.backend.ParticipantPartyRecordStorageBackend

import scala.concurrent.{ExecutionContext, Future}

object PersistentParticipantPartyRecordStore {
...
class PersistentParticipantPartyRecordStore(
    dbSupport: DbSupport,
    metrics: Metrics,
  ...
  private def doCreatePartyRecord(
    ...
    val dbParty = ParticipantPartyRecordStorageBackend.DbPartyRecordPayload(
  ...
  private def doUpdatePartyRecord(
      dbPartyRecord: ParticipantPartyRecordStorageBackend.DbPartyRecord,
      partyRecordUpdate: PartyRecordUpdate,
  )(connection: Connection): Unit = {
    val now = epochMicroseconds()
    ...
    //       updating this user record for the rest of the transaction.
  ...
  private def toDomainPartyRecord(
      payload: ParticipantPartyRecordStorageBackend.DbPartyRecordPayload,
  ...
  private def withPartyRecord[T](
      ...
      f: ParticipantPartyRecordStorageBackend.DbPartyRecord => T
