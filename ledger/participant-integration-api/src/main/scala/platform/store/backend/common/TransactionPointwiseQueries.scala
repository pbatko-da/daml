...
class TransactionPointwiseQueries(
  ...
  // TODO pbatko: Remove me
  object ForTesting {
    def fetchFlatEvents(transactionId: Ref.TransactionId, requestingParties: Set[Party])(
        connection: Connection
    ): Vector[EventStorageBackend.Entry[Raw.FlatEvent]] = {
      fetchIdsFromTransactionMeta(transactionId)(connection) match {
        case Some((first, last)) =>
          fetchFlatTransactionEvents(first, last, requestingParties)(connection)
        case None => sys.error(s"Transaction $transactionId not found")
      }
    }

    def fetchTreeEvents(transactionId: Ref.TransactionId, requestingParties: Set[Party])(
        connection: Connection
    ): Vector[EventStorageBackend.Entry[Raw.TreeEvent]] = {
      fetchIdsFromTransactionMeta(transactionId)(connection) match {
        case Some((first, last)) =>
          fetchTreeTransactionEvents(first, last, requestingParties)(connection)
        case None => sys.error(s"Transaction $transactionId not found")
      }
    }
