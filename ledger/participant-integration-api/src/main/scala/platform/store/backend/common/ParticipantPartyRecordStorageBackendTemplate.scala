...
class ParticipantPartyRecordStorageBackendTemplate extends ParticipantPartyRecordStorageBackend {
  ...
  override def getPartyRecord(
      party: Ref.Party
  )(connection: Connection): Option[ParticipantPartyRecordStorageBackend.DbPartyRecord] = {
    SQL"""
         SELECT
             internal_id,
             party,
      ...
      .map { case (internalId, party, resourceVersion, createdAt) =>
        ParticipantPartyRecordStorageBackend.DbPartyRecord(
  ...
  override def createPartyRecord(
      partyRecord: ParticipantPartyRecordStorageBackend.DbPartyRecordPayload
