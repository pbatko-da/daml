...
private[apiserver] final class ApiPartyManagementService private (
    ...
    participantPartyRecordStore: ParticipantPartyStore,
...
//        }
  ...
  private def handleResult[T](operation: String)(
      result: v2.ParticipantPartyStore.Result[T]
  )(implicit errorLogger: DamlContextualizedErrorLogger): Future[T] =
    result match {
      case Left(ParticipantPartyStore.PartyNotFound(party)) =>
        Future.failed(
      ...
      case Left(ParticipantPartyStore.PartyExists(party)) =>
        Future.failed(
      ...
      case Left(ParticipantPartyStore.ConcurrentPartyUpdateDetected(party)) =>
        Future.failed(
