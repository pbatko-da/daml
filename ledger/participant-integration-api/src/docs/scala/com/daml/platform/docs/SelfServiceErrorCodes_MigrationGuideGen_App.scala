...
object SelfServiceErrorCodes_MigrationGuideGen_App {
  def main(args: Array[String]): Unit = {
    ...
    //    if (args.length < 10) System.exit(0)

    //    println(genErrorCategoryTableText())
    println("\n\n########################## LEDGER API")
    println(genStatusCodeMigrationTableText("com/daml/platform/docs/self-service-error-codes-migration.yml"))

    println("\n\n########################## SANDBOX classic")
    println(genStatusCodeMigrationTableText("com/daml/platform/docs/self-service-error-codes-sandbox-classic-migration.yml"))

    println("\n\n########################## KV (Daml Sandbox and VMBC)")
    println(genStatusCodeMigrationTableText("com/daml/platform/docs/self-service-error-codes-kv-errors-migration.yml"))
  ...
  def generateReStTable(rows: Array[Array[String]], header: Array[String]): String = {
    ...
    case class Cell(lines: mutable.ArrayBuffer[String]) {
      ...
      def padRight(targetLength: Int, targetHeight: Int): Cell = {
        require(targetHeight >= this.height(), s"Cell ${this} has more lines than the max lines expected: ${targetHeight}!")
    ...
    val textTableRows: Array[String] = table.map { row =>
      ...
      if (row.length == 1) {
        ...
        row.head.padRight(targetLength = totalLineWidthExclusiveOfOuterBorders, row.head.height()).lines.map(l => s"|${l}|").mkString("\n")
      } else {
