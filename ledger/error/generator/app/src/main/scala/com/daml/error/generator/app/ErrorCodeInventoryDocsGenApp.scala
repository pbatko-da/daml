...
import com.daml.error.generator.{ErrorDefinitionsDocParser, ErrorCodeDocItem, ErrorGroupDocItem}
...
object ErrorCodeInventoryDocsGenApp {
  ...
  def main(args: Array[String]): Unit = {
    val text = {
      val errorDocItems = ErrorDefinitionsDocParser.getErrorCodeItems()
      val groupDocItems = ErrorDefinitionsDocParser.getErrorGroupItems()
      ...
      val errorCodes: Seq[ErrorCodeValue] = errorDocItems.map {
        errorCodeDocItem: ErrorCodeDocItem =>
          ErrorCodeValue(
            ...
            deprecationO = errorCodeDocItem.deprecation.map(v => newlineIntoSpace(v.message)),
