...
import com.daml.error.generator.ErrorDefinitionsDocParser.DeprecatedItem
...
@nowarn
class ErrorDefinitionsDocParserSpec extends AnyFlatSpec with Matchers {
  ...
  it should "return the correct doc items from the error classes" in {
    ...
    val actualGroupDocItems = ErrorDefinitionsDocParser.getErrorGroupItems(searchPackages)
    val actualErrorDocItems = ErrorDefinitionsDocParser.getErrorCodeItems(searchPackages)
    ...
    val expectedErrorDocItems = Seq(
      ...
      ErrorCodeDocItem(
        ...
        deprecation = Some(DeprecatedItem(since = "since 123", message = "deprecated.")),
        explanation = Some(Explanation("Things happen.")),
        resolution = Some(Resolution("Turn it off and on again.")),
      ),
      ErrorCodeDocItem(
  ...
  it should "parse annotations of an error category" in {
    val actual = ErrorDefinitionsDocParser.getErrorCategoryItem(TransientServerFailure)
