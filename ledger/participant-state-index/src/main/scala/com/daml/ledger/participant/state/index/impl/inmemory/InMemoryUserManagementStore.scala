...
import com.daml.ledger.participant.state.index.v2.{ObjectMetaUpdate, UserManagementStore, UserUpdate}
...
class InMemoryUserManagementStore(createAdmin: Boolean = true) extends UserManagementStore {
  ...
  override def createUser(user: User, rights: Set[UserRight])(implicit
    ...
    withoutUser(user.id) {
      val userWithResourceVersion = user.copy(metadata = user.metadata.copy(resourceVersionO = Some("0")))
